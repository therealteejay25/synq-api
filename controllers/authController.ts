...
export const requestMagicLink = async (req : Request<{}, {}, MagicLinkRequest>, res : Response) => {
    try {
        ...
        if (!email) {
            ...
            res.status(200).json({ message: "Magic link sent!", magicLinkUrl, user, token })
...
export const verifyMagicLink = async (req: Request, res: Response) => {
    try {
        ...
        const hashed = crypto.createHash("sha256").update(token as string).digest("hex");
